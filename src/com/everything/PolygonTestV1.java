 ...
 * Time: 11:38 AM
 */
public class PolygonTestV1 {
    int    boothSides = 4;
//    float  polyX[];
//    float  polyY[];
    List<GeoZonePoint> parent;

//    float  x, y;
    BigDecimal[]  constant = new BigDecimal[4];
    BigDecimal[]  multiple = new BigDecimal[4];

    PolygonTestV1(List<GeoZonePoint> p){
        this.parent = p;
    }

    void check(List<GeoZonePoint> ch){
        precalc_values();
        System.out.println("precalc done");
        int count=0;
        for(int i = 0;i<boothSides;i++){
            if(pointInPolygon(ch.get(i).rawx,ch.get(i).rawy)){
                count++;
            }
        }
...
//        child.add(new GeoZonePoint(new BigDecimal(6), new BigDecimal(3)));
        ...
        PolygonTestV1 p = new PolygonTestV1(parent);
        p.check(child);
//        p.precalc_values();
